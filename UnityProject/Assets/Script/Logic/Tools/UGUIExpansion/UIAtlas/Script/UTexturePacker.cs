...
namespace UGUI
{
    public class UTexturePacker
    {
        ...
        static public Rect[] PackTextures(Texture2D texture, Texture2D[] textures, int width, int height, int padding, int maxSize, bool forceSquareAtlas = true)
        {
            if (width > maxSize && height > maxSize) return null;
            if (width > maxSize || height > maxSize) { int temp = width; width = height; height = temp; }

            // Force square by sizing up
            if (forceSquareAtlas == true)
            {
                if (width > height)
                    height = width;
            ...
            UTexturePacker bp = new UTexturePacker(width, height, false);
                ...
                for (xPadding = 1; xPadding >= 0; --xPadding)
                {
                    for (yPadding = 1; yPadding >= 0; --yPadding)
                    {
                        rect = bp.Insert(tex.width + (xPadding * padding), tex.height + (yPadding * padding),
                            UTexturePacker.FreeRectChoiceHeuristic.RectBestAreaFit);
