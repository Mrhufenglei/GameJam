...
namespace UGUI
{
    public abstract class UTweener : MonoBehaviour
    {
                ...
                if (current == null)
                {
                    ...
                    if (onFinished != null)
                    {
                        ...
                        for (int i = 0; i < mTemp.Count; ++i)
                        {
                            ...
                            if (ed != null && !ed.oneShot) UEventDelegate.Add(onFinished, ed, ed.oneShot);
                        }
                        mTemp = null;
                    }
        ...
        List<UEventDelegate> mTemp = null;

        /// <summary>
        /// Convenience function -- set a new OnFinished event delegate (here for to be consistent with RemoveOnFinished).
        /// </summary>
        public void SetOnFinished(UEventDelegate.Callback del) { UEventDelegate.Set(onFinished, del); }

        /// <summary>
        /// Convenience function -- set a new OnFinished event delegate (here for to be consistent with RemoveOnFinished).
        /// </summary>
        public void SetOnFinished(UEventDelegate del) { UEventDelegate.Set(onFinished, del); }

        /// <summary>
        /// Convenience function -- add a new OnFinished event delegate (here for to be consistent with RemoveOnFinished).
        /// </summary>
        public void AddOnFinished(UEventDelegate.Callback del) { UEventDelegate.Add(onFinished, del); }

        /// <summary>
        /// Convenience function -- add a new OnFinished event delegate (here for to be consistent with RemoveOnFinished).
        /// </summary>
        public void AddOnFinished(UEventDelegate del) { UEventDelegate.Add(onFinished, del); }
