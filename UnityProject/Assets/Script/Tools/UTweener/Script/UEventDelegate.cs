...
namespace UGUI
{
    ...
    public class UEventDelegate
    {
        ...
        public UEventDelegate(Callback call) { Set(call); }
        public UEventDelegate(MonoBehaviour target, string methodName) { Set(target, methodName); }
	...
	static bool IsValid (Callback callback) { return callback != null; }
        ...
        public override bool Equals(object obj)
        {
            ...
            {
                UEventDelegate del = obj as UEventDelegate;
                return (mTarget == del.mTarget && string.Equals(mMethodName, del.mMethodName));
            }
            return false;
        }

        static int s_Hash = "UEventDelegate".GetHashCode();
        ...
        static public bool IsValid(List<UEventDelegate> list)
        {
            if (list != null)
            {
        ...
        static public UEventDelegate Set(List<UEventDelegate> list, Callback callback)
        {
            if (list != null)
            {
                UEventDelegate del = new UEventDelegate(callback);
                list.Clear();
                list.Add(del);
                return del;
            }
        ...
        static public void Set(List<UEventDelegate> list, UEventDelegate del)
        {
            if (list != null)
            {
                list.Clear();
        ...
        static public UEventDelegate Add(List<UEventDelegate> list, Callback callback) { return Add(list, callback, false); }

        /// <summary>
        /// Append a new event delegate to the list.
        /// </summary>

        static public UEventDelegate Add(List<UEventDelegate> list, Callback callback, bool oneShot)
        {
            if (list != null)
            {
                for (int i = 0, imax = list.Count; i < imax; ++i)
                ...
                UEventDelegate ed = new UEventDelegate(callback);
                ed.oneShot = oneShot;
                list.Add(ed);
                return ed;
            }
        ...
        static public void Add(List<UEventDelegate> list, UEventDelegate ev) { Add(list, ev, ev.oneShot); }

        /// <summary>
        /// Append a new event delegate to the list.
        /// </summary>

        static public void Add(List<UEventDelegate> list, UEventDelegate ev, bool oneShot)
        {
            ...
            else if (list != null)
            {
                ...
                UEventDelegate copy = new UEventDelegate(ev.target, ev.methodName);
        ...
        static public bool Remove(List<UEventDelegate> list, Callback callback)
        {
            if (list != null)
            {
                for (int i = 0, imax = list.Count; i < imax; ++i)
        ...
        static public bool Remove(List<UEventDelegate> list, UEventDelegate ev)
        {
            if (list != null)
            {
                for (int i = 0, imax = list.Count; i < imax; ++i)
