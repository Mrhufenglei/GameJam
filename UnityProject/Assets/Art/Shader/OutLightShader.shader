	...
	SubShader
	{
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
                ...
                //漫反射颜色和强度 
                //fixed3 diffuse = lambert * _LightColor0.rgb * pow(saturate(dot(worldNormal, worldLightDir)), _Gloss) + ambient;
				...
				////进行纹理采样  
				fixed4 color = tex2D(_MainTex, i.uv) * fixed4(diffuse.rgb,1) + _TextureColor;
				//以下为本篇主题：计算RimLight  
				//把视线方向归一化  
				...
				//计算rimLight  
				fixed3 rimColor = 0;//_RimColor * pow(rim, 1 / _RimPower) * 3;
				if (_RimPower > 0)
				{
					 rimColor = _RimColor * pow(rim, 1 / _RimPower) * 3;
				}
				//输出颜色+边缘光颜色  
